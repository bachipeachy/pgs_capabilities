"""
runtime.py — Execution-only host for CS_SEND_EMAIL_V0.

Responsibilities:
- Execute email side-effect operations
- Translate backend errors into CEP-visible result_status values
- Skip gracefully when SMTP is not configured (testbed-safe)
"""
...
from pgs_side_effects.implementation.side_effects.external.CS_SEND_EMAIL_V0.impl.executor import (
    SendEmailEngine,
)


class SendEmailRuntime:
    """Execution-only host for CS_SEND_EMAIL_V0."""

    capability_kind = "CS"
    _default_capability_code = "CS_SEND_EMAIL_V0"
    ...
    def __init__(self, config: Dict[str, Any], metadata: Dict[str, Any], capability_code: str | None = None):
        ...
        self._engine = SendEmailEngine(config)
